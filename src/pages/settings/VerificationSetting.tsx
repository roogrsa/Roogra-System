...
export default function VerificationSetting() {
    ...
    const upateVerification = async (
        ...
        try {
            setSubmitting(true);
            const res = await axiosInstance.put(`/api/verification_settings/1`, values);
            console.log(res);
        } catch (error: any) {
            console.error(error);
        ...
        <>
            ...
            >
                {({ isSubmitting }: FormikProps<VerificationValues>) => (
                    ...
                    <Form>
                        <div className="">
                            <InputText type='number' isReq min={0} name='commercial_register_fee' label={t(`settings.commercial`)} />
                            <InputText type='number' isReq min={0} name='identity_document_fee' label={t(`settings.maroof`)} />
                            <InputText type='number' isReq min={0} name='freelancer_document_fee' label={t(`settings.self-employment`)} />
                            <InputText type='number' isReq min={0} name='verification_fee' label={t(`settings.subscription`)} />
                            <InputText type='number' isReq min={0} name='subscription_fee' label={t(`settings.verification-check-fees`)} />
                        </div>
